...
В других языках мы сами указываем, когда нам нужно такое поведение. Но в JS это по умолчанию.

Если бы весь

Если бы поток выполнения ждал пока выполнится
...
##Кроссбраузерность
Мы не будем рассматривать эксцентричное поведение некоторх браузеров.
