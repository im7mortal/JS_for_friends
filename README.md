...
Для полноценного функционирования объекта нужно связать методы и свойства объекта.

Для этого существует ссылка внутри объекта на самого себя. Ссылка определяется ключевым словом this.

```
var object = {
	name: 'lol', // свойство объекта
	getName: function () {
		return this.name; // возвращает свойство данного объекта
	}
}
console.log('Name is ' + object.getName() + '!')
```

Здесь есть подводные камни, но мы рассмотрим их позже.
...
Помните про подводные камни с использование this.
this это ссылка обьекта на самого себя. Но в то же время в js все является обьектом. Но главная опасность таится применения this
в контексте выполнения функции.

Когда мы обьявляем метод обьекта, автоматически создается область видимости этого метода, но ссылка this по прежнему указывает
на объект владелец метода. Опасность возникает в случае применения функций высшего порядка.
...
Но для начала выясним как традиционный стиль эмулируется через прототипное наследование.
...
Результат выше будет неожиданным даже для некоторых программистов, которые хорошо знают js.(иллюстрация трудноотлавлиемой ошибки)

Вот пример поочевидней.

```
var object = {
	fg: 'lol', // свойство объекта
	getName: function () {
		return function () {
				return this.fg;
			}
